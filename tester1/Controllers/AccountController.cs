...
namespace tester1.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        private async Task<ObjectResult> GenerateToken(ApplicationUser user)
        {
            //start token generation
            // create secret key
            var secretKey = new SymmetricSecurityKey(Encoding.UTF8.GetBytes("here we have to add a key that is larger than 16 characters"));
        ...
        [HttpPost]
        public async Task<IActionResult> SepcialRegister([FromBody] RegisterVM model)
        {
            ApplicationUser user = await _context.ApplicationUsers.FindAsync(model.UserId);
            if (user != null)
            {
            ...
            var result = await _userManager.ChangePasswordAsync(user, "", model.Password);
